...
| **L3.5** | 燒錄字幕 / 螢幕文字（OCR） | ★ ★ ★ ★ ☆（強，補 L3 空缺） | 畫面內崁的文字（作者寫進畫面的重點），可對時間戳；純視覺 / 零口白 reel 唯一的文字訊號 |
...
**這時要靠 L3.5 + L4 補**：純視覺敘事 reel 的「主題真相」靠 L4 VLM，畫面內崁的重點文字靠 L3.5 OCR。

---

### L3.5 — 燒錄字幕 / 螢幕文字（OCR，補 L3 空缺）

| 你能看到的 | 限制 |
|-----------|------|
| 畫面內崁的字幕 / 大字 hook / 標註 | 只有畫面有寫才抓得到 |
| 帶時間戳的螢幕文字 | OCR 準確度依引擎 / 畫質 |
| 產品名 / 價格 / 步驟標號等硬資訊 | 藝術字 / 動態字可能漏 |

**L3.5 為什麼獨立一層**：短影音常把訊息「寫進畫面」而不是（或同時不只）用講的。這層文字——
- **比 L2 caption 可靠**：它在影片「裡面」，不是作者為了 reach 另外寫的觀點
- **可對時間戳**：跟 L3 transcript 一樣能定位到秒，不像 L2 是整體
- **直接補 L3 的空缺**：慢節奏 / 純 BGM / 零口白 reel（L3 給不出料）的唯一文字訊號

**Reel Scout 怎麼跑 L3.5**（§4F）：
- 預設 `OCR_ENGINE=vlm` — 直接復用 VLM 已經讀進 `text_in_frame` 的螢幕文字，**零額外依賴**
- 可選 `OCR_ENGINE=tesseract` — 專用 OCR 引擎跑 keyframe（CJK 更強，但屬 opt-in 重依賴，預設不啟；裝不到就退回 vlm）
- 存進 `ocr_captions` 表（帶 `timestamp_sec` + `engine` 出處），merge 階段當「On-screen Text (L3.5)」餵進分析

**典型用途**：edocolala 黑膠那種 L3 弱的片，若畫面有內崁標題 / 曲目字卡，L3.5 就把它撈出來補上主題線索。
