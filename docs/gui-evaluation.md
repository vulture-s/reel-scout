...
> **⚡ 更新（2026-07-17，v1.1.0）— conditional-go 已觸發、已建。**
> 用一堂實際課程（6/28 逐字稿 + retro）回審後，最小形態被**重新校準**：真實用途不是「Hevin 的日常 review dashboard」（他走 CLI+MCP），而是 **① 課堂 live demo 面（升級 deck 靜態截圖）+ ② 學員回去也能用的檢視器**。因學員多為不裝 reel-scout 的新手，「回去可用」= 必須**零安裝/自包含**。
> **成本修正**：本文原估「~1 天」對照 decision-log 舊估「40–80hr」——那是**打包 app**；實際唯讀檢視器復用 v1.0 的 `db`/`stats`/`compare`，兩面共 ~1.5 天。
> **已交付（v1.1.0）**：`export --format html`（自包含單檔、零安裝 take-home）+ `reel-scout view`（本機 server、live demo），共用一個 renderer。**硬守楔子：唯讀、無任何動作按鈕，分數標「參照非權威」。** 見 CHANGELOG 1.1.0。
...
| 最小唯讀本機檢視器（`export --format html` + `reel-scout view`，只讀 DB） | ✅ **Go（已建，v1.1.0）** | 觸發條件＝課堂 live demo + 學員 take-home（見文首更新）；唯讀、零動作按鈕，守住教學楔子；復用 v1.0 資料層，~1.5 天非 40–80hr |
